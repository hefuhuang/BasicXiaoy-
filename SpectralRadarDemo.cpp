// SpectralRadarDemo.cpp : Definiert den Einstiegspunkt f黵 die Konsolenanwendung.
//
...
void ContinuousBScanMeasurement(bool rotate)//连续的扫描测量
{
	char message[1024];
...
void AveragingAndBinning()
{
    ...
    while(!_kbhit())
    {
        ...
        exportColoredData(ColBScan, ColoredDataExport_JPG, "test\\test2.jpg");
...
void ExportImportTest()
{
    ...
    exportData1D(Tmp, Data1DExport_TXT, "test\\Apodization.txt");

    getCalibration(Proc, Calibration_OffsetErrors, Tmp);
    exportData1D(Tmp, Data1DExport_TXT, "test\\OffsetErrors.txt");

    getCalibration(Proc, Calibration_Chirp, Tmp);
    exportData1D(Tmp, Data1DExport_TXT, "test\\Chirp.txt");
    ...
    while(!_kbhit())
    {
        ...
        exportRawData(Raw, RawDataExport_RAW, "test\\InFocus.raw");
        ...
        exportData2D(BScan, Data2DExport_SRM, "test\\test.srm");
        exportData2D(BScan, Data2DExport_RAW, "test\\test.raw");

        importData(Import, DataImport_SRM, "test");
...
void RawData(void)
{
    ...
    while(!_kbhit())
    {
        ...
        exportComplexData(CData, ComplexDataExport_RAW, "test\\complex_disp.raw");
...
void RawDataExportImportTest()
{
	...
	exportRawData(Raw, RawDataExport_SRR, "Test\\Raw.srr");
	clearRawData(Raw);

	Raw = createRawData();
	importRawData(Raw, RawDataImport_SRR, "Test\\Raw.srr");
	exportRawData(Raw, RawDataExport_SRR, "Test\\Raw_copy.srr");
	clearRawData(Raw);
...
void OfflineProcessingTest()
{
	...
	saveCalibration(ProcTmp, Calibration_Chirp, "tmp\\Chirp.dat");
	saveCalibration(ProcTmp, Calibration_OffsetErrors, "tmp\\Offset.dat");
	saveCalibration(ProcTmp, Calibration_ApodizationSpectrum, "tmp\\Spectrum.dat");
	...
	exportData2D(DataTmp, Data2DExport_RAW, "tmp\\res_orig.raw");
	clearData(DataTmp);

	closeProcessing(ProcTmp);

	closeDevice(Dev);
	exportRawData(RawTmp, RawDataExport_SRR, "tmp\\raw.srr");
	clearRawData(RawTmp);
	...
	loadCalibration(Proc, Calibration_Chirp, "tmp\\Chirp.dat");
	loadCalibration(Proc, Calibration_OffsetErrors, "tmp\\Offset.dat");
	loadCalibration(Proc, Calibration_ApodizationSpectrum, "tmp\\Spectrum.dat");

	RawDataHandle Raw = createRawData();
	importRawData(Raw, RawDataImport_SRR, "tmp\\raw.srr");
...
{  
	...
	cout << "1: Continuous B-Scan acquisition, rotated by 90癨n";
